...
// Update an existing interest record
router.put('/:userIntErndId',
    [
        param('userIntErndId')
            .isInt({ min: 1 }).withMessage('UserIntErndID must be a positive integer(be)'),
        body('CompanyName')
            .trim().notEmpty().withMessage('Company name cannot be empty(be)')
            .isLength({ max: 50 }).withMessage('Company name must be 50 characters or less(be)'),
        body('ContractNumber')
        ...
        body('Notes')
            ...
            .isLength({ max: 2000 }).withMessage('Notes must be 2000 characters or less(be)'),
        handleValidationErrors,
        auth
    ],
    async (req, res) => {
        ...
        try {
            const [result] = await pool.query(
                `UPDATE InterestEarnedT
                 SET CompanyName = ?,
                     ContractNumber = ?,
                     DateOpened = ?,
                     Rate = ?,
                     Notes = ?
                 WHERE UserIntErndID = ? AND UserID = ?`,
                [
                    CompanyName,
                    ContractNumber,
                    DateOpened,
            ...
            res.json({
                ...
                UserIntErndID: Number(userIntErndId)
            });
        } catch (error) {
            if (error.code === 'ER_DUP_ENTRY') {
                return res.status(400).json({ error: 'Contract already exists' });
            }
            console.error('Error updating interest record:', error.message);
            res.status(500).json({ error: 'Database error' });
        }
    }
);

// Delete an existing interest record
router.delete('/:userIntErndId',
    [
        param('userIntErndId')
            .isInt({ min: 1 }).withMessage('UserIntErndID must be a positive integer(be)'),
        handleValidationErrors,
    ...
    async (req, res) => {
        ...
        try {
            const [result] = await pool.query(
                'DELETE FROM InterestEarnedT WHERE UserIntErndID = ? AND UserID = ?',
                [userIntErndId, userId]
            );
            ...
            console.error('Error deleting interest record:', error.message);
            res.status(500).json({ error: 'Database error' });
        }
    }
);
