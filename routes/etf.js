...
// ────────────────────────────────────────────────
// PUT /etf/activity/:id
// Edit a single transaction (date, shares, cost/price only — type and symbol are immutable)
router.put(
  '/activity/:id',
  auth,
  [
    body('TransactionDate').isDate(),
    body('Shares').isFloat({ gt: 0 }),
    body('PurchaseCost').optional().isFloat({ gt: 0 }),
    body('SalePrice').optional().isFloat({ gt: 0 })
  ],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    const { TransactionDate, Shares, PurchaseCost, SalePrice } = req.body;
    ...
    try {
      await withTransaction(async (connection) => {
        ...
        // For BUY edits: ensure updated shares would not make inventory negative
        if (tx.TransactionType === 'BUY') {
          const shareDiff = parseFloat(Shares) - parseFloat(tx.Shares);
          if (shareDiff < 0) {
            const [sellRows] = await connection.query(
              `SELECT COALESCE(SUM(Shares), 0) AS totalSold
               FROM etfActivityT
               WHERE etfSymbolID = ? AND UserID = ? AND TransactionType = 'SELL'`,
              [tx.etfSymbolID, userId]
            );
            const [buyRows] = await connection.query(
              `SELECT COALESCE(SUM(Shares), 0) AS totalBought
               FROM etfActivityT
            ...
            const newInventory = parseFloat(buyRows[0].totalBought) + shareDiff
                               - parseFloat(sellRows[0].totalSold);
            if (newInventory < 0) {
              throw new Error("Cannot reduce shares: would make inventory negative (be)");
            }
          }
          if (!PurchaseCost) {
            throw new Error("PurchaseCost required for BUY transaction (be)");
          }
          await connection.query(
            `UPDATE etfActivityT
             SET TransactionDate = ?, Shares = ?, PurchaseCost = ?
             WHERE etfActivityID = ? AND UserID = ?`,
            [TransactionDate, Shares, PurchaseCost, activityId, userId]
          );
        } else {
          // SELL: verify updated shares don't exceed inventory
          const [buyRows] = await connection.query(
            `SELECT COALESCE(SUM(Shares), 0) AS totalBought
             FROM etfActivityT
             WHERE etfSymbolID = ? AND UserID = ? AND TransactionType = 'BUY'`,
          ...
          const currentInventory = parseFloat(buyRows[0].totalBought)
                                 - (parseFloat(sellRows[0].totalSold) - parseFloat(tx.Shares));
          if (parseFloat(Shares) > currentInventory) {
            throw new Error(`Cannot sell more shares than held. Available: ${currentInventory.toFixed(6)} (be)`);
          }
          if (!SalePrice) {
            throw new Error("SalePrice required for SELL transaction (be)");
          }
          await connection.query(
            `UPDATE etfActivityT
             SET TransactionDate = ?, Shares = ?, SalePrice = ?
             WHERE etfActivityID = ? AND UserID = ?`,
            [TransactionDate, Shares, SalePrice, activityId, userId]
          );
        }
      ...
      logger.error(`PUT /etf/activity/:id: ${err.message}`);
      res.status(500).json({ message: "Server error (be)" });
    }
  }
);
