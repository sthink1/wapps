...
- **Email:** Resend, using the verified `wonderfulappscompany.com` sending domain
...
- General usage tracking/analytics, including administrator App Usage, User Usage, and service-capacity/free-tier monitoring
...
| `/track` | `routes/track.js` | Authenticated usage tracking; administrator-only analytics and free-tier/capacity endpoints |
...
### Current hosted database capacity

The deployed FreeSQLdatabase instance currently has a **5 MB** database-size allowance. `routes/track.js` measures current MySQL size from `information_schema.TABLES` by summing `DATA_LENGTH + INDEX_LENGTH`, with `SHOW TABLE STATUS` as a fallback.

A capacity check performed on September 19, 2026 reported:

```text
Data:    1.406 MB
Indexes: 2.984 MB
Total:   4.391 MB
Used:    87.8% of 5.00 MB
Remaining: approximately 0.61 MB
```

The largest table at that snapshot was `TrackUsageT` at approximately **0.719 MB** with about **6,875 rows**. Most of the total database footprint is index/storage overhead rather than raw application data. Treat these values as a dated capacity snapshot, not hard-coded application constants.

Current immediate paid fallback recorded for planning purposes is **100 MB for $19**. Provider pricing can change and must be rechecked before purchase.
...
`TrackUsageT` remains the detailed page/time tracking table. `UserUsageT` and `TrackUsageT` remain distinct data sources, but the administrator dashboard deliberately combines them for reporting.

### 6.8 Administrator usage and service-capacity dashboard

`httpdocs/track.html` is now an administrator-focused operational dashboard rather than only a legacy statistics page. `routes/track.js` enforces administrator access for the dashboard data endpoints.

Current dashboard areas are:

1. **App Usage** – an application-centered matrix with one Description column plus the active applications from `AppT`. Metrics include Visits, Active Users, Sessions, Total Time Used, Average Session, Requests / Actions, Data Activity, Email Activity, Storage Activity, and Last Used.
2. **User Usage** – the same style of usage reporting for one user, selectable by dropdown or direct UserID entry.
3. **Free Tier Usage** – service-capacity monitoring for the currently deployed infrastructure, including immediate paid fallback information.

Both App Usage and User Usage support independent period filters:
...
For Visits, `routes/track.js` uses the larger of recorded `TrackUsageT` page views or `UserUsageT` `APP_OPEN` events. This avoids under-counting applications whose legacy pages do not independently write `TrackUsageT` View records.

The Free Tier Usage endpoint is:

```text
GET /track/free-tier-usage
```

It currently monitors or displays planning values for:

- **FreeSQLdatabase** – current data + index size against the 5 MB free limit.
- **Resend** – emails sent today and this calendar month, retrieved from the Resend metrics API.
- **Render** – 750 free instance-hours/month; current provider usage remains a provider-dashboard value.
- **Cloudflare R2** – current stored bytes measured from the configured bucket; Class A/Class B operation totals remain provider-dashboard values; Internet egress is shown as free.

Dashboard status thresholds are:

```text
under 70%       OK
70% through 84.9%  Caution
85% and above   Warning
```

The dashboard also carries the immediate paid fallback so approaching a free-tier limit can be handled before service interruption. Current planning values reviewed September 19, 2026 are:

| Service/resource | Free allowance | Immediate paid fallback | Current planning cost |
|---|---:|---|---:|
| FreeSQLdatabase | 5 MB | 100 MB | $19 |
| Resend | 100/day and 3,000/month | Pro, 50,000/month, no daily limit | $20/month |
| Render web service | 750 instance-hours/month | Starter / 0.5 CPU, 512 MB RAM | $7/month |
| R2 storage | 10 GB-month | Usage above free allowance | $0.015/GB-month |
| R2 Class A | 1,000,000/month | Usage above free allowance | $4.50/million |
| R2 Class B | 10,000,000/month | Usage above free allowance | $0.36/million |
| R2 Internet egress | Free | No upgrade required | Free |

Treat provider pricing as dated operational configuration. Recheck pricing before an upgrade or production capacity decision. Keep these values centralized in backend reporting/configuration rather than duplicating them in multiple frontend files.
...
### 7.4 Email delivery configuration

WA-generated email is sent through **Resend**. The verified sending domain is `wonderfulappscompany.com`, and production is configured so application mail can use:

```text
MAIL_FROM_ADDRESS=noreply@wonderfulappscompany.com
```

`MAIL_TO_ADDRESS` is the destination mailbox for Contact Us submissions and is currently configured to a Gmail mailbox that is read through Thunderbird. Contact Us mail sets the visitor's address as `reply_to`, so replying from the receiving mail client addresses the visitor rather than the no-reply sender.

The previous x10 mailbox/domain is no longer part of WA's normal application email path and should not be included in WA service-capacity/free-tier reporting unless it is deliberately reintroduced.

Automated notification email uses the central `send_email.js` flow and includes `WonderfulAppsCompany.com` in the footer. Optional-category messages continue to include the appropriate unsubscribe link.

### 7.5 Family Tree notifications
...
- UI-disabled/hidden buttons are a convenience only; the backend must remain authoritative. `Track Activity` is hidden from non-admin users in `home.html`, but administrator enforcement remains server-side in `routes/track.js`.
...
- `home.html` and `FamilyTree.html` use information-icon patterns to expose concise descriptions without changing the primary navigation action. Preserve the separation between the main action and the information control.
...
14. If changing tracked infrastructure, verify `/track/free-tier-usage`, provider limits, and paid-fallback values.
15. Update `wappsDump.sql` for intentional schema changes.
16. Update `CLAUDE.md` and `TechSummary.md` for architectural changes.
...
- Do not treat dated provider pricing/free-tier limits as permanent; verify them before relying on them for an upgrade decision.
- Do not reintroduce x10 as a WA email dependency unless intentionally required.
