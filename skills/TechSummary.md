...
- Axios `^1.13.5`
- Resend `^6.4.2`, using the verified `wonderfulappscompany.com` sending domain
...
| `/track` | `routes/track.js` | Authenticated tracking plus administrator App Usage, User Usage, and free-tier/capacity reporting |
...
### 4.9 Current hosted database capacity snapshot

The production FreeSQLdatabase account currently allows **5 MB**. The administrator tracking route measures database size from `information_schema.TABLES` as data plus indexes, with `SHOW TABLE STATUS` as fallback.

A September 19, 2026 measurement was:
...
| Indexes | 2.984 MB |
| Total | 4.391 MB |
| Percent of 5 MB used | 87.8% |
| Approximate remaining | 0.61 MB |

At that snapshot, `TrackUsageT` was the largest table at approximately **0.719 MB** and about **6,875 rows**. The database is therefore capacity-constrained primarily because the 5 MB free tier is very small relative to a 53-table InnoDB application; index/storage overhead is a major part of the current footprint.

The immediate paid fallback currently recorded for operational planning is **100 MB for $19**. This is provider pricing/configuration information, not a database-schema rule.
...
### 6.8 Usage tracking and administrator dashboard

`UserUsageT` stores application/subscription-related events. Allowed event types currently include:
...
`TrackUsageT` stores page views and time-spent records. The two tables remain distinct, but `routes/track.js` deliberately combines them for administrator reporting.

`httpdocs/track.html` now contains three operational sections:

- **App Usage** – application-centered usage matrix.
- **User Usage** – per-user drill-down with dropdown and direct UserID entry.
- **Free Tier Usage** – current infrastructure capacity and paid-fallback planning.

App Usage and User Usage each support independent Today, Last 7 Days, Last 30 Days, and All Time filters. Visits use the greater of tracked page views and `APP_OPEN` events so applications without legacy page-view logging are not shown as unused after being opened.

Administrator endpoints include:

```text
GET /track/app-usage
GET /track/users
GET /track/user-usage
GET /track/free-tier-usage
```

The Free Tier section currently covers FreeSQLdatabase, Resend, Render, and Cloudflare R2. x10 is intentionally excluded because it is no longer part of the normal WA email path.

Current capacity/planning values reviewed September 19, 2026:

| Service/resource | Free allowance | Paid fallback | Cost | Measurement source |
|---|---:|---|---:|---|
| FreeSQLdatabase size | 5 MB | 100 MB | $19 | Automatic MySQL size query |
| Resend daily sends | 100/day | Pro | $20/month | Resend metrics API |
| Resend monthly sends | 3,000/month | 50,000/month; no daily limit | included in Pro | Resend metrics API |
| Render instance time | 750 hours/month | Starter, 0.5 CPU / 512 MB | $7/month | Provider dashboard |
| R2 storage | 10 GB-month | usage based | $0.015/GB-month | Automatic current bucket bytes |
| R2 Class A | 1,000,000/month | usage based | $4.50/million | Provider dashboard |
| R2 Class B | 10,000,000/month | usage based | $0.36/million | Provider dashboard |
| R2 Internet egress | Free | none required | Free | Included |

Status thresholds are under 70% = OK, 70% through 84.9% = Caution, and 85% or higher = Warning. Provider prices and allowances are dated operational data and should be reverified before an upgrade.
...
All WA-generated application email is sent through Resend. The sending domain `wonderfulappscompany.com` is verified with Resend/DNS and production uses the no-reply sender through environment configuration:

```text
MAIL_FROM_ADDRESS=noreply@wonderfulappscompany.com
```

Contact Us mail is also transmitted by Resend. `MAIL_TO_ADDRESS` points to the receiving Gmail mailbox, while the visitor's email is assigned as `reply_to`, allowing a normal Reply from Thunderbird or another mail client to address the visitor.

The x10 mailbox/domain is no longer part of the normal WA application-email path and is omitted from the Free Tier Usage dashboard.

Central notification emails include `WonderfulAppsCompany.com` in the footer. Optional notification categories continue to include their unsubscribe handling.

### 7.5 Family Tree integration
...
- Preserve administrator-only presentation for Track Activity in `home.html`; hiding the button is not the security boundary.
- Preserve the separate information-icon action used by `home.html` and `FamilyTree.html` when editing those navigation buttons.
...
- `JWT_SECRET`;
- `ADMIN_USERNAME`;
- Resend/API credentials and sender/recipient variables (`MAIL_FROM_NAME`, `MAIL_FROM_ADDRESS`, `MAIL_TO_ADDRESS`);
- object-storage credentials;
- `APP_TIMEZONE` when used for provider/day-boundary reporting;
...
### Operational provider monitoring

`routes/track.js` contains the free-tier/capacity reporting logic. `r2Storage.js` exposes current storage statistics used by the dashboard. Provider-account metrics that are not reliably available inside WA remain labeled as provider-dashboard values rather than being estimated.

`track.html` includes a manual **REFRESH FREE TIER USAGE** action. The purpose is operational planning and service continuity: approaching limits should be visible early enough to move to the recorded paid fallback without disrupting development or production.

Pricing/fallback data must be reviewed periodically because it is external provider information and can change independently of the codebase.
...
[ ] If service-capacity logic changes, verify /track/free-tier-usage and current provider fallback values
...
15. Administrator usage reporting combines `UserUsageT` and `TrackUsageT` intentionally without collapsing them into one data model.
16. Free-tier and paid-fallback figures are operational configuration and must be reviewed before upgrade decisions.
17. x10 is not part of the current WA application-email path.
18. Record major architecture changes in both `CLAUDE.md` and `TechSummary.md`.
